  ...
  constructor() {
    ...
    this.ringback = deviceId => {
      ...
      if (deviceId !== 'default') {
        ...
        if (mediaElement) {
          const _dest = Tone.context.createMediaStreamDestination();
...
class SIPAccount {
  ...
  makeCall(number) {
    const state = phoneStore.getState();
    const sessionsLimit = state.config.phoneConfig.sessionsLimit;
    const sessionsActiveObject = state.sipSessions.sessions;
    const strictMode = state.config.appConfig.mode;
    const attendedTransfersActive = state.sipSessions.attendedTransfers.length;
    const sessionsActive = Object.keys(sessionsActiveObject).length;
    const sessionDiff = sessionsActive - attendedTransfersActive;
...
class Dialstring extends Component {
  ...
  handleDial() {
    const sessionsActive = Object.keys(this.props.sessions).length;
    const attendedTransferActive = this.props.attendedTransfersList.length;
    const sessionDiff = sessionsActive - attendedTransferActive;
